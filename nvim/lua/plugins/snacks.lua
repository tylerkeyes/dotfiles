...
return {
  {
    ...
    opts = {
      ...
      profiler = {
        runtime = "~/Software/neovim/runtime/",
        presets = {
          on_stop = function()
      ...
      gitbrowse = {
        open = function(url)
          vim.fn.system(" ~/Software/folke-dot/config/hypr/scripts/quake")
          vim.ui.open(url)
        end,
      },
    },
    ...
    keys = {
      ...
      { "<leader>t", function() Snacks.scratch({ icon = " ", name = "Todo", ft = "markdown", file = "~/Software/folke-dot/TODO.md" }) end, desc = "Todo List" },
      {
        "<leader>dpd",
        desc = "Debug profiler",
        function()
